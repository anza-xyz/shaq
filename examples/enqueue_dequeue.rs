    ...
    let consumer_handle = std::thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                ...
                run_spsc_consumer(consumer, exit);
            }
        })
        .unwrap();
    ...
    let producer_handle = std::thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                ...
                run_spsc_producer(producer, exit, verbose, total_items_produced);
            }
        })
        .unwrap();

    run_total_throughput_loop::<Item>(exit.clone(), total_items_produced);
...
fn run_spsc_consumer(mut consumer: SpscConsumer<Item>, exit: Arc<AtomicBool>) {
...
fn run_mpmc(producers: usize, consumers: usize, verbose: bool) {
    ...
    for idx in 0..consumers {
        ...
        let core_id = consumer_cores[idx];
        handles.push(
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    run_mpmc_consumer(consumer, exit);
                })
                .unwrap(),
        );
    }
    ...
    for idx in 0..producers {
        let exit = exit.clone();
        let file = queue_file.try_clone().unwrap();
        let core_id = producer_cores[idx];
            ...
            std::thread::Builder::new()
                ...
                .spawn({
                    ...
                    move || {
                        ...
                        run_mpmc_producer(producer, exit, report_prefix, total_items_produced);
                    }
                })
                .unwrap(),
        );
...
fn run_mpmc_consumer(consumer: MpmcConsumer<Item>, exit: Arc<AtomicBool>) {
    run_consumer_loop(exit, move || {
